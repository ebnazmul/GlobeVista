...
const Card2 = ({ post }) => {
  ...
  const deletePost = () => {
    axios
      .delete(`https://glovevista-server.vercel.app/mylists/delete/${post._id}`)
      .then(() => {
    ...
    <div className="border p-4 my-2 flex justify-between">
      ...
      <div className={`py-3 px-4 border ${!deletePrompt && "hidden"}`}>
        ...
        <div>
          <button
            onClick={deletePost}
            className="px-2 py-1 rounded bg-red-400 text-white mr-4">
            Yes
          </button>
          <button
      ...
      <div className="*:py-1 *:px-2 *:bg-blue-400 text-gray-100 *:rounded">
        <button className="mr-2">
          <Link to={`/mylists/update/${post._id}`}>Update</Link>
        </button>
